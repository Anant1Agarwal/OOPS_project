...
@FXML private Button store_btn, history_btn, account_btn, enquiry_btn;
    ...
    ObservableList<String> productItems = productNameChoice.getItems();
if (!productItems.isEmpty()) {
    productNameChoice.setValue(productItems.get(2));
     productSizeChoice.setValue(productSizeChoice.getItems().get(1));
}
...
if(cartTable.getSelectionModel().getSelectedItem()==null){
    ...
    public void handleCheckout(ActionEvent event) {
        try {
            ...
            e.printStackTrace(); // Handle the exception appropriately (log it, show an alert, etc.)
        }
    }
